...
echo "" && echo "uv export --locked --format=requirements.txt --no-dev --no-emit-project > /tmp/requirements.txt"
uv export --locked --format=requirements.txt --no-dev --no-emit-project --output-file /tmp/requirements.txt
...
echo "" && echo "python manage.py compilemessages"
